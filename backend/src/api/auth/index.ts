...
router.post("/login", async (req: Request, res: Response) => {
  ...
  if (user.locked)
    return res
      .status(400)
      .json({ message: "Your account is locked!", success: false });
  ...
  if (!user.verified) {
    ...
    sendVerificationEmail(
      { to: user.email },
      await genConfirmationCode(user.id)
    );
...
router.post("/register", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // return res
    //   .status(400)
    //   .json({ data: null, message: e.errors[0], success: false });
    return res.status(400).json({ data: null, message: e, success: false });
  }
  ...
  if (user)
    return res
      .status(400)
      .json({ data: null, message: "User already exists!", success: false });
  // hash password
  ...
  sendVerificationEmail(
    ...
    await genConfirmationCode(createdUser.id)
  );
...
router.put("/confirm/", async (req: Request, res: Response) => {
  ...
  if (!code)
    return res
      .status(400)
      .json({ data: null, message: "No code provided", success: false });
...
router.post("/reset/password/new", async (req: Request, res: Response) => {
  ...
  return res.status(200).json({
    message: "Successfully sent link for changing password!",
    success: true,
  });
});
...
router.put("/reset/password/", async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .json({ message: "Successfully updated password!", success: true });
});
