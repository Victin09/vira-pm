...
router.post("/:guildId/join", async (req: Request, res: Response) => {
  ...
  return res.status(200).json({
    message: `Successfully joined the guild ${guild.name}`,
    success: true,
  });
});
...
router.get("/:guildId/channels", async (req: Request, res: Response) => {
  ...
  if (!guildId)
    return res
      .status(400)
      .json({ message: "Search parameter not specified!", success: false });
  ...
  return res.status(200).json({
    data: channels,
    message: "Successfully found all channels!",
    sucess: true,
  });
});
...
router.post("/:guildId/channels", async (req: Request, res: Response) => {
  ...
  if (!guildId || isNaN(parseInt(guildId)))
    return res.status(400).json({
      message: "Search parameter not specified or is invalid!",
      success: false,
    });
  ...
  if (!guild)
    return res
      .status(400)
      .json({ message: "Guild not found!", success: false });

  if (guild.ownerId !== userId)
    return res
      .status(400)
      .json({ message: "Only owners can create channels!", success: false });
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json({ message: "Successfully delete channel!", success: true });
  }
);
