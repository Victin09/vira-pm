...
const MyCalendar: React.FC = () => {
  ...
  // const DateHeaderContent: React.FC<any> = (props) => {
  //   const handleCick = () => {
  //     setModalData(props.date)
  //   }

  //   return (
  //     <div>
  //       <label htmlFor="createEventModal" className="hover:cursor-pointer">{props.label}</label>
  //     </div>
  //   )
  // }
  ...
  const handleDateClick = (currentDate: Date) => {
    ...
    setShowModal(true)
    console.log('currentDateHandleClick', currentDate)
  };
      ...
      <BoxWrapper>
        <Calendar
          ...
          components={{
            ...
            // ),
            // month: {
            //   dateHeader: DateHeaderContent
            // }
          }}
        />
      </BoxWrapper>
      {showModal && (
        <CreateEventModal currentDate={modalData!} handleClick={() => setShowModal(false)} />
      )}
    </>
  );
};
